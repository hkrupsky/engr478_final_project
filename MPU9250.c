...
static void write_preset(uint8_t preset[][2], int write_count)
{
	...
	if(write_count == 0) {
		...
		if(array_end < write_count) {
			#ifdef DEBUG_PRINT
			UARTprintf("Incorrect parameter for write_preset. Write abort.\r\n");
			#endif
			return;
		}
	}
...
static void calibrate_mag(MPU_param_t *MPU_param){
    ...
    delay(10);                                                          // long wait between AK8963 mode changes
		...
		#ifdef DEBUG_PRINT
		char msg[40];
		UARTprintf("ASAX data from AK8963:\r\n");
		for(i = 0; i < 3; i++) {
...
static void preconfig(MPU_param_t *MPU_param, BOOL fast_config) {
	...
	if(!whoami_MPU()) {
		UARTprintf("Wrong MPU device ID.\r\n");
		LEDS = 0x02;
		exit(EXIT_FAILURE);
	}
	if(!whoami_AK8963()) {
		UARTprintf("Wrong AK8963 device ID.\r\n");
		LEDS = 0x02;
		exit(EXIT_FAILURE);
	}
